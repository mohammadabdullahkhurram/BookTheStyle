    ...
    <body class="min-h-screen bg-paper text-ink antialiased" @if ($bodyTheme = \App\Support\AppTheme::current($salon)) data-theme="{{ $bodyTheme }}" @endif>
        ...
        >
            ...
            <div class="flex min-w-0 flex-1 flex-col">
                ...
                @if ($salon?->is_demo)
                    {{-- The demo banner: an honest guest-preview notice and a
                         sales asset. No reset — the showcase is shared and
                         read-only; the CTAs are the real login and a call. --}}
                    <div role="status" class="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-[#E3D9A8] bg-[#FBF6DC] px-4 py-2.5 text-[13.5px] text-[#5C531F] sm:px-6">
                        <p class="min-w-0">
                            <span class="font-semibold">{{ __('You\'re viewing a live demo.') }}</span>
                            {{ __('A read-only preview of a working salon dashboard — everything here is fictional.') }}
                        </p>
                        <span class="flex shrink-0 items-center gap-3">
                            <a href="{{ route('login') }}" class="font-semibold underline underline-offset-2 transition hover:opacity-75">{{ __('Log in') }}</a>
