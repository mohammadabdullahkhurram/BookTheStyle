...
@if ($preview ?? false)
    {{-- LIVE-DRAFT BRIDGE (preview mode only): the customizer posts the
         draft design here; we re-theme WITHOUT reloading or persisting by
         rewriting CSS custom properties and swapping the logo/theme nodes.
         The colour derivations mirror AccentPalette::resolve() and
         WidgetBranding::mode() exactly, so the live preview matches what a
         real save would render. Same-origin only — the origin is checked. --}}
    <script>
    (function () {
        'use strict';
        var FONTS = @json(\App\Support\WidgetBranding::FONTS);
        var overrides = document.createElement('style');
        overrides.id = 'bts-preview-overrides';
        document.head.appendChild(overrides);

        function mix(hex, target, weight) {
            var h = function (s, i) { return parseInt(s.slice(i, i + 2), 16); };
            var out = '#';
            for (var i = 1; i < 7; i += 2) {
                var v = Math.round(h(hex, i) + (h(target, i) - h(hex, i)) * weight);
        ...
        function onColor(bg) { return contrast(bg, '#FFFFFF') >= contrast(bg, '#1C1B1A') ? '#FFFFFF' : '#1C1B1A'; }

        function apply(draft) {
            var accent = draft.accent;
            var surface = draft.surface;
            var dark = contrast(surface, '#FFFFFF') >= contrast(surface, '#1C1B1A');
            var font = FONTS[draft.font] || FONTS.editorial;
            var css = ':root{'
                + '--brand-accent:' + accent + ';--accent:' + accent + ';'
                + '--brand-accent-hover:' + mix(accent, '#000000', 0.14) + ';--accent-hover:' + mix(accent, '#000000', 0.14) + ';'
                + '--brand-accent-tint:' + mix(accent, '#FFFFFF', 0.88) + ';--accent-tint:' + mix(accent, '#FFFFFF', 0.88) + ';'
                + '--brand-accent-ink:' + mix(accent, '#000000', 0.30) + ';--accent-ink:' + mix(accent, '#000000', 0.30) + ';'
                + '--brand-accent-foreground:' + onColor(accent) + ';--wb-accent-ink:' + onColor(accent) + ';'
                + '--wb-secondary:' + draft.secondary + ';'
                + '--wb-surface:' + surface + ';'
                + '--wb-ink:' + (dark ? '#FFFFFF' : '#1C1B1A') + ';'
                + '--wb-muted:' + (dark ? 'rgb(255 255 255 / .74)' : 'rgb(28 27 26 / .68)') + ';'
                + '--wb-faint:' + (dark ? 'rgb(255 255 255 / .42)' : 'rgb(28 27 26 / .38)') + ';'
                + '--wb-line:' + (dark ? 'rgb(255 255 255 / .16)' : 'rgb(28 27 26 / .12)') + ';'
                + '--wb-cell:' + (dark ? 'rgb(255 255 255 / .07)' : 'rgb(255 255 255 / .72)') + ';'
                + '--wb-display:' + font.display + ';'
                + '--wb-body:' + font.body + ';'
                + '}';
            overrides.textContent = css;

            if (draft.theme) { document.body.dataset.theme = draft.theme; }

            var img = document.querySelector('.wb-logo');
            var title = document.querySelector('.wb-info h1');
            if (draft.logoUrl) {
                if (!img) {
                    img = document.createElement('img');
                    img.className = 'wb-logo';
                    img.alt = '';
        ...
        window.addEventListener('message', function (event) {
            ...
            if (!event.data || event.data.type !== 'bts-widget-draft') { return; }
            apply(event.data);
        });
    })();
    </script>
